...
- $Lemma4:$带有加法的n维向量的群$\mathbb { M_n}$，和带有加法的在$H$上的最高项次数不大于n-1的单变量多项式$\mathbb H_p^{(\leq n-1)} [X]$的群，映射：$h:\mathbb M_n \to \mathbb H_p^{(\leq n-1)}$，令$L_i(X)为Lagrange Basis$，有$h(\vec m)= \langle \vec m,\vec L_i(X) \rangle$，是群同态
- $Lemma5:$同态映射的复合映射必定是同态映射
- $由Lemma1,Lemma4,Lemma5,有$

$$
\vec{q_O}\circ \vec{w}=\vec{q_L}\circ\vec{u}+\vec{q_R}\circ\vec{v}+\vec{q_M}\circ(\vec{u}\circ\vec{v})+\vec{q_C}\circ\vec{c} \\ \iff \langle \vec q_O,\vec L_i(X) \rangle \cdot \langle \vec w,\vec L_i(X) \rangle=\langle \vec q_L,\vec L_i(X) \rangle \cdot \langle \vec u,\vec L_i(X) \rangle+\langle \vec q_R,\vec L_i(X) \rangle \cdot \langle \vec v,\vec L_i(X) \rangle+\langle \vec q_m,\vec L_i(X) \rangle \cdot (\langle \vec u,\vec L_i(X) \rangle\cdot\langle \vec v,\vec L_i(X) \rangle )+\langle \vec q_C,\vec L_i(X) \rangle 
$$

令$a(X) =\langle \vec a,\vec L_i(X) \rangle$ ，上式转化为：

$$
q_O(X)w(X)=q_L(X)u(X)+q_R(X)v(X)+q_M(X)u(X)v(X)+q_C(X)
$$

- 至此，n个算术约束转化成了八个n-1次多项式之间的一个运算关系。
