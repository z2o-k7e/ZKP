...
- **Wiring(Copy Constraints)**
    ...
    - Wiring实现思路：
        ...
        - 至此，问题转化成如何证明连乘等式 $b_0\cdot b_1\cdot b_2\cdot ...b_{n-1}=c$，即证明一个n步递归，
            ...
            - 至此，**Wiring**转化成三个约束
                - $r_0=1和r_n=c$  即约束向量的指定位的值为h，即约束$\vec r$的第1位($r_0$)的值为1，第n+1位($r_{n+1}$)的值为$c$。
...
- 带有加法的n维向量的群$\mathbb { A_n}$和最高项次数不大于n-1的单变量多项式$\mathbb F_p^{(\leq n-1)} [X]$的群，映射：$h:\mathbb A_n \to \mathbb P^{\leq n-1}$，有$h(\vec a)=\left \langle \vec a,\vec X^i \right \rangle$，是群同态
